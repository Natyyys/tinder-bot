...
class MyTelegramBot extends HtmlTelegramBot {
    ...
    async opener(msg) {
        ...
        this.user = {}
        this.count = 0;
        await this.sendText("Имя собеседника?")
    }
    ...
    async openerDialog(msg) {
        ...
        if(this.count === 1) {
            this.user["name"] = text;
            await this.sendText("Сколько лет собеседнику?")
        }

        if(this.count === 2) {
            this.user["age"] = text;
            await this.sendText("Оцените внешность собеседника: 1 - 10 баллов?")
        }

        if(this.count === 3) {
            this.user["handsome"] = text;
            await this.sendText("Кем работает ваш собеседник?")
        }
...
const bot = new MyTelegramBot("7216636371:AAFJlDgG69vQfZBzTysBVYKmPrzzc0OaJdA");
